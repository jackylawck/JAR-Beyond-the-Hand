/**
 * 大氣環境粒子與流動能量槽 (Zero-GC Atmosphere Engine)
 */
export class AtmosphereFX {
    constructor(scene) {
        ...
        for (let i = 0; i < this.dustCount; i++) {
            this.dustPositions[i * 3] = (Math.random() - 0.5) * 6.5;
            this.dustPositions[i * 3 + 1] = Math.random() * 3.5;
            this.dustPositions[i * 3 + 2] = (Math.random() - 0.5) * 6.5;
        ...
        for (let i = 0; i < 8; i++) {
            ...
            this.scene.add(node);
            this.energyNodes.push({ mesh: node, angle: (i / 8) * Math.PI * 2, radius: 2.35, speed: 0.85 });
        }
    }
    ...
    update(dt) {
        ...
        for (let i = 0; i < this.dustCount; i++) {
            ...
            if (pos[idx + 1] > 3.8) {
                pos[idx + 1] = 0.1;
                pos[idx] = (Math.random() - 0.5) * 6.5;
                pos[idx + 2] = (Math.random() - 0.5) * 6.5;
            }
        }
